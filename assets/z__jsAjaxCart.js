...
window.PXUTheme.jsAjaxCart = {
  ...
  updateView: function () {
    ...
    (function() {
      ...
      {
        $('.us-only').addClass('visible');
      }
      if (geoplugin_countryCode() != 'US')
      {
        $('.international-only').addClass('visible');
      }
    })();
    ...
    .done(({ html, options }) => {
      ...
      if (options.item_count > 0) {
        ...
        const mm_fship_us = $(html.content).find('.mm_us_fs');
        const mm_fship_int = $(html.content).find('.mm_international_fs');
        $('.mm_us_fs').replaceWith(mm_fship_us);
        $('.mm_international_fs').replaceWith(mm_fship_int);
